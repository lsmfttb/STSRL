"""Offline T040 expert non-combat source-coverage comparison command."""
...
from sts_combat_rl.sim.expert_source_coverage import (
    ...
    dump_expert_source_coverage_comparison_report_json,
)


def run_expert_source_coverage_report_from_paths(
    *,
    output_path: Path,
    arm_specs: Sequence[Sequence[str]],
) -> ExpertSourceCoverageComparisonReport:
    """Load T040 arm artifacts, build the comparison, and write JSON output."""

    arm_inputs = []
    for spec_index, spec in enumerate(arm_specs):
        if len(spec) != 3:
            raise ValueError(f"expert source arm {spec_index} must have 3 values")
        role, pool_raw, coverage_raw = spec
        ...
        arm_inputs.append((role, pool, coverage_report, artifact_identity))

    report = build_expert_source_coverage_comparison_report(arm_inputs)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    with output_path.open("w", encoding="utf-8", newline="\n") as stream:
        dump_expert_source_coverage_comparison_report_json(report, stream)
    return report
