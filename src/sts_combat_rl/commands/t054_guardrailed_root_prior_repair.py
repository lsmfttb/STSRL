"""Command helpers for the T054 guardrailed root-prior repair report."""
...
from sts_combat_rl.sim.t054_guardrailed_root_prior_repair import (
    ...
    build_t054_retention_manifest_payload,
    dump_t054_guardrailed_root_prior_repair_report_json,
    dump_t054_retention_manifest_json,
    format_t054_guardrailed_root_prior_repair_report,
    format_t054_retention_manifest,
)


def run_t054_guardrailed_root_prior_repair_from_paths(
    *,
    artifact_specs: Sequence[Sequence[str]],
    output_path: Path,
) -> T054GuardrailedRootPriorRepairReport:
    """Load retained/generated artifacts, build the T054 report, and write it."""

    artifacts = _verified_artifacts(artifact_specs)
    by_role = {artifact["role"]: artifact for artifact in artifacts}
    with Path(by_role["t052_root_prior_guided_comparison"]["path"]).open(
        "r",
        encoding="utf-8",
    ) as stream:
        t052_comparison = load_root_prior_guided_search_comparison_jsonl(stream)
    with Path(by_role["t052_result_summary"]["path"]).open(
        "r",
        encoding="utf-8",
    ) as stream:
        t052_result_summary = json.load(stream)
    if not isinstance(t052_result_summary, dict):
        raise ValueError("T052 result summary must be a JSON object")
    with Path(by_role["t053_failure_analysis"]["path"]).open(
        "r",
        encoding="utf-8",
    ) as stream:
    ...
    with output_path.open("w", encoding="utf-8", newline="\n") as stream:
        dump_t054_guardrailed_root_prior_repair_report_json(report, stream)
    return report


def run_t054_retention_manifest_from_paths(
    *,
    output_path: Path,
    artifact_specs: Sequence[Sequence[str]],
    ...
    """Build and write a T054 retention manifest for generated artifacts."""
    ...
    manifest = build_t054_retention_manifest_payload(
    ...
    output_path.parent.mkdir(parents=True, exist_ok=True)
    with output_path.open("w", encoding="utf-8", newline="\n") as stream:
        dump_t054_retention_manifest_json(manifest, stream)
    return manifest


def format_t054_guardrailed_root_prior_repair_command(
    report: T054GuardrailedRootPriorRepairReport,
) -> str:
    """Format the T054 path-level report command output."""

    return format_t054_guardrailed_root_prior_repair_report(report)


def format_t054_retention_manifest_command(payload: dict[str, Any]) -> str:
    """Format the T054 retention manifest command output."""

    return format_t054_retention_manifest(payload)


def _verified_artifacts(
    artifact_specs: Sequence[Sequence[str]],
) -> list[dict[str, Any]]:
    if len(artifact_specs) != len(T054_REQUIRED_INPUT_ROLES):
        raise ValueError(
            "T054 requires exactly six --t054-input-artifact values: "
            + ", ".join(T054_REQUIRED_INPUT_ROLES)
        )
    artifacts = []
    for spec in artifact_specs:
        if len(spec) != 3:
            raise ValueError("--t054-input-artifact requires ROLE PATH SHA256")
    ...
    for role in T054_REQUIRED_INPUT_ROLES:
        if role not in roles:
            raise ValueError(f"missing required T054 input artifact role {role}")
    if len(set(roles)) != len(roles):
        raise ValueError("duplicate T054 input artifact roles")
    return artifacts


def _retained_artifact_payload(spec: Sequence[str]) -> dict[str, Any]:
    if len(spec) != 3:
        raise ValueError("--t054-retained-artifact requires ROLE PATH SCHEMA_ID")
...
def _command_payload(spec: Sequence[str]) -> dict[str, str]:
    if len(spec) != 2:
        raise ValueError("--t054-retention-command requires ROLE COMMAND")
...
def _stage_payload(spec: Sequence[str]) -> dict[str, Any]:
    if len(spec) != 5:
        raise ValueError(
            "--t054-retention-stage requires ROLE WORKERS SHARDS RECORD_RANGE SECONDS"
        )
...
def _note_payload(spec: Sequence[str]) -> tuple[str, str]:
    if len(spec) != 2:
        raise ValueError("--t054-retention-note requires KEY VALUE")
