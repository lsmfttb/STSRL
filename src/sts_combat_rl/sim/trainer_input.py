...
TRAINER_INPUT_DATASET_FORMAT_VERSION = 1


@dataclass(frozen=True)
class TrainerInputRecord:
...
def dump_trainer_input_dataset_jsonl(
    ...
    stream.write(
        json.dumps(_metadata_row(dataset), sort_keys=True, separators=(",", ":"))
    )
    stream.write("\n")
...
def load_trainer_input_dataset_jsonl(stream: TextIO) -> TrainerInputDataset:
    ...
    for line_number, raw_line in enumerate(stream, start=1):
        ...
        if row_type == "metadata":
            ...
            records.append(_record_from_dict(record_raw))
        else:
...
def _record_from_pair(
    ...
    return TrainerInputRecord(
        ...
        snapshot_features=[float(value) for value in example.snapshot_features],
        legal_action_features=[
            [float(value) for value in action_features]
