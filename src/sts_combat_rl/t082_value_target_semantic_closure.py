...
EXPECTED_INPUTS = {
    ...
    "../t042-assisted-source-scale-pr39/runs1000_s20_workers16/scale-manifest.json": (
        "25efae30dc9a61c8b97cb09e1844b93b9ffe693bde51c0f494f0f65203a1d327",
        "t042-assisted-source-scale-manifest-v2",
    ),
}
CLASSIFICATIONS = {
...
def _identity(action: Any) -> dict[str, Any] | None:
    ...
    if not isinstance(action.get("stable_id"), str) or not action["stable_id"]:
        return None
    if (
        isinstance(action.get("occurrence"), bool)
        or not isinstance(action.get("occurrence"), int)
        or action["occurrence"] < 0
    ):
        return None
    return dict(action)
...
def recover_behavior(
    current: Mapping[str, Any], successor: Mapping[str, Any] | None
) -> dict[str, Any]:
    if successor is None:
        return {
            "status": "unavailable",
            "reason": "final/no immediate record",
    ...
    for key in ("source_run_id", "source_seed"):
        if (key in current or key in successor) and successor.get(key) != current.get(
            key
        ):
            return {
                "status": "unavailable",
                "reason": f"malformed/{key} linkage mismatch",
                "successor_exists": True,
            }
    if (
        "source_battle_index" in current or "source_battle_index" in successor
    ) and successor.get("source_battle_index") != current.get(
        "source_battle_index", -1
    ) + 1:
        return {
            "status": "unavailable",
            "reason": "non-adjacent battle",
            "successor_exists": True,
        }
    current_meta = current.get("structural_metadata", {})
    successor_meta = successor.get("structural_metadata", {})
    if (
        isinstance(current_meta, Mapping)
        and isinstance(successor_meta, Mapping)
        and current_meta.get("assistance_level")
        != successor_meta.get("assistance_level")
    ):
        return {
            "status": "unavailable",
            "reason": "malformed/component linkage mismatch",
            "successor_exists": True,
        }
    ...
    return {
        "status": "available",
        "identity": after_ids[len(before_ids)],
    ...
    with path.open(encoding="utf-8") as stream:
        for number, line in enumerate(stream, 1):
            ...
            if not isinstance(item, Mapping):
                raise TypeError(f"invalid JSONL row {number}")
    ...
    with path.open(encoding="utf-8") as stream:
        for number, line in enumerate(stream, 1):
            ...
            if not isinstance(row, Mapping):
                raise TypeError(f"malformed non-object source row {number}")
            index = row.get("record_index")
            if not isinstance(index, int) or index <= previous_index:
                raise ValueError(
                    f"malformed or duplicate source record_index in {path}"
                )
...
def _validate_pool(
    path: Path, expected: Mapping[str, Any], component: str
) -> dict[str, Any]:
    digest = hashlib.sha256()
    size = 0
    count = 0
    ...
    with path.open("rb") as stream:
        for raw in iter(lambda: stream.readline(), b""):
            ...
            try:
                ...
                continue
            if not isinstance(item, Mapping):
                reason = "pool envelope row is not an object"
                continue
            if item.get("type") == "metadata":
                metadata_count += 1
                if not isinstance(item.get("metadata"), Mapping):
                    ...
                    schema = metadata.get("schema_id")
                continue
            row = item.get("record", item)
            if (
                not isinstance(row, Mapping)
                or not isinstance(row.get("structural_metadata"), Mapping)
                or row["structural_metadata"].get("assistance_level") != component
            ):
                duplicate = True
                continue
            index = row.get("record_index")
            if (
                ...
                duplicate = True
            previous = index if isinstance(index, int) else previous
            count += 1
            if (
                row.get("structural_metadata", {}).get("assistance_level", component)
                != component
            ):
                duplicate = True
    valid = (
        reason is None
        and metadata_count == 1
        and schema == expected.get("schema_id")
        and metadata.get("format_version") == expected.get("format_version")
        and metadata.get("record_count") == count
        and metadata.get("assistance_level") == component
        and count == expected.get("record_count")
...
def _record_identity(
    ...
    if metadata.get("assistance_level") != component:
        raise ValueError(
            "source assistance_level is missing or does not match component"
        )
    obj = SimpleNamespace(
...
def _linkage_ok(
    ...
    checks = {
        ...
        "teacher_source": all(
            teacher.get(key) == identity.get(key)
            for key in (
                "source_checkpoint_id",
                "source_run_id",
                "source_seed",
...
def _load_envelope(path: Path) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    with path.open(encoding="utf-8") as stream:
        for number, line in enumerate(stream, 1):
            item = json.loads(line)
            if not isinstance(item, Mapping):
                raise TypeError(f"envelope row is not an object: {path}:{number}")
            if item.get("type") == "metadata":
                metadata_count += 1
                if not isinstance(item.get("metadata"), Mapping):
                    raise ValueError(f"metadata is not an object: {path}:{number}")
                metadata = dict(item["metadata"])
...
def _load_selected_envelope(
    path: Path, expected: int, index_field: str
        ...
        for number, line in enumerate(stream, 1):
            ...
            if item.get("type") == "metadata":
                ...
                if isinstance(index, int) and 0 <= index < expected:
                    if index in rows:
                        raise ValueError(f"duplicate {index_field}: {index}")
                    rows[index] = row
                    expected_next += 1
            else:
                raise ValueError(f"invalid envelope row {number}")
...
def audit_t064(
    manifest_path: Path, output: Path, *, expected_rows: int = 460
) -> dict[str, Any]:
    ...
    def incomplete(problem: str) -> dict[str, Any]:
        report = {
            ...
            "qualification_mode": "formal_460"
            if expected_rows == 460
            else "compact_non_qualifying",
            "execution": {
                "mode": "offline_streaming",
                "worker_count": 1,
                "reason": "non-simulator aggregation/single stream",
            },
            "regeneration": {
                "command": f"PYTHONPATH=src python scripts/run_t082_value_target_semantic_closure.py --manifest {manifest_path} --output {output}"
            },
            "inputs": {
                "manifest": {
                ...
                "control_artifacts": input_checks,
                "pool_checks": [
                    {"valid": False, "reason": "unavailable before pool read"}
                ],
                "teacher": {
                    "path": str(manifest_path.parent / "teacher/merged.jsonl"),
                    "valid": False,
                    "reason": "unavailable before teacher read",
                },
                "trainer": {
                    "path": str(manifest_path.parent / "trainer/trainer-input.jsonl"),
                    "valid": False,
                    "reason": "unavailable before trainer read",
                },
                "terminal_case_valid": False,
            },
            "integrity": {"valid": False, "problems": [problem]},
            "rows": [],
        }
        report["classification"] = "INCOMPLETE"
        report["inputs"]["teacher"]["observed"] = None
        report["inputs"]["trainer"]["observed"] = None
        report["inputs"]["pool_checks"] = [
            {
                "component": c,
                "observed": None,
                "valid": False,
                "reason": "unavailable before pool read",
            }
            for c in (
                "assist_0",
                "assist_hp50",
        ...
        report["regeneration"]["command"] = (
            f"PYTHONPATH=src python scripts/run_t082_value_target_semantic_closure.py --manifest {manifest_path} --output {output}"
        )
        report["inputs"]["pool_checks"] = [
            {
                "component": c,
        ...
        report["inputs"]["terminal"] = {
            "observed": None,
            "valid": False,
            "reason": "unavailable before decision read",
        }
        output.write_text(
        ...
        input_checks.append(
            {
                ...
                and document.get("schema_id") == schema,
            }
        )
    ...
    terminal_valid = (
        ...
        and decision.get("terminal_case") == "Case B"
    )
    selected = (
        manifest.get("selected_sources", []) if isinstance(manifest, Mapping) else []
    )
    if len(selected) != expected_rows or any(
        not isinstance(item, Mapping)
        or not isinstance(item.get("complete_identity"), Mapping)
        or item.get("complete_identity", {}).get("schema_id")
        != "t064-complete-source-identity-v1"
        or not isinstance(item.get("complete_identity_sha256"), str)
        for item in selected
    ):
        return incomplete(
            f"selected inventory does not contain exactly {expected_rows} valid rows"
        )
    if any(
        not isinstance(item.get("component"), str)
        or not isinstance(item.get("source_record_index"), int)
        or isinstance(item.get("source_record_index"), bool)
        or not isinstance(item.get("source_path"), str)
        or not item["source_path"]
        for item in selected
    ):
        return incomplete(
            "selected inventory has missing or malformed component/index/source_path"
        )
    ...
    if (
        ...
        or not required_components.issubset(artifacts)
        or any(
            item["component"] not in artifacts
            or not isinstance(artifacts[item["component"]], Mapping)
            or not isinstance(artifacts[item["component"]].get("path"), str)
            or not artifacts[item["component"]].get("path")
            or not isinstance(artifacts[item["component"]].get("sha256"), str)
            or len(artifacts[item["component"]]["sha256"]) != 64
            or any(
                isinstance(artifacts[item["component"]].get(k), bool)
                or not isinstance(artifacts[item["component"]].get(k), int)
                or artifacts[item["component"]].get(k) < 0
                for k in ("bytes", "record_count", "format_version")
            )
            or not isinstance(artifacts[item["component"]].get("schema_id"), str)
            for item in selected
        )
    ):
        return incomplete(
            "selected component has missing or malformed input artifact specification"
        )
    selected_hashes = [item["complete_identity_sha256"] for item in selected]
    selected_indexes = [
        (item.get("component"), item.get("source_record_index")) for item in selected
    ]
    if len(set(selected_hashes)) != len(selected_hashes) or len(
        set(selected_indexes)
    ) != len(selected_indexes):
        return incomplete(
            "selected inventory contains duplicate identity hashes or component/index pairs"
        )
    ...
    teacher_path, trainer_path = (
        root / "teacher/merged.jsonl",
        root / "trainer/trainer-input.jsonl",
    )
    try:
        teacher_envelope_meta, teachers = _load_selected_envelope(
            teacher_path, expected_rows, "row_index"
        )
        trainer_envelope_meta, trainers = _load_selected_envelope(
            trainer_path, expected_rows, "example_index"
        )
    except (
        OSError,
        ValueError,
        ...
        [
            {
                ...
                "schema": teacher_envelope_meta.get("artifact_schema_id"),
                "record_count": teacher_envelope_meta.get("record_count"),
                "controller_provenance": teacher_envelope_meta.get(
                    "controller_provenance"
                ),
                "valid": (
                    expected_rows != 460 or sha256(teacher_path) == EXPECTED_TEACHER_SHA
                )
                and teacher_envelope_meta.get("artifact_schema_id")
                == "oracle-search-teacher-v1"
                and teacher_envelope_meta.get("record_count") == expected_rows
                and isinstance(
                    teacher_envelope_meta.get("controller_provenance"), Mapping
                )
                and teacher_envelope_meta["controller_provenance"]
                .get("config", {})
                .get("information_regime")
            ...
            },
            {
                ...
                "expected_sha256": EXPECTED_TRAINER_SHA,
                "schema": trainer_envelope_meta.get("format_version"),
                "record_count": trainer_envelope_meta.get("record_count"),
                "policy_target_schema_id": trainer_envelope_meta.get(
                    "policy_target_schema_id"
                ),
                "policy_target_schema_version": trainer_envelope_meta.get(
                    "policy_target_schema_version"
                ),
                "structured_battle_outcome_schema_id": trainer_envelope_meta.get(
                    "structured_battle_outcome_schema_id"
                ),
                "structured_battle_outcome_schema_version": trainer_envelope_meta.get(
                    "structured_battle_outcome_schema_version"
                ),
                "valid": (
                    expected_rows != 460 or sha256(trainer_path) == EXPECTED_TRAINER_SHA
                )
                and trainer_envelope_meta.get("record_count") == expected_rows
                and trainer_envelope_meta.get("format_version") in (6, "6")
                and trainer_envelope_meta.get("policy_target_schema_id")
                == "trainer-policy-target-v1"
                and trainer_envelope_meta.get("policy_target_schema_version") == 1
                and trainer_envelope_meta.get("structured_battle_outcome_schema_id")
                == "structured-battle-outcome-v1"
                and trainer_envelope_meta.get(
                    "structured_battle_outcome_schema_version"
                )
                == 1,
            },
    ...
    for index, item in enumerate(selected):
        by_component[item["component"]].add(item["source_record_index"])
    pool_checks = []
                ...
                pool_checks.append(
                    {
                        "component": component,
                        "valid": False,
                        "reason": "malformed input artifact specification",
                    }
                )
                continue
                ...
                pool_checks.append(
                    {
                        ...
                        "component": component,
                        "valid": False,
                        "reason": f"malformed artifact: {exc}",
                    }
                )
    ...
    for component, indexes in by_component.items():
        previous = None
        pool = Path(
            manifest["input_artifacts"][component]["path"]
            .replace("D:\\", "/mnt/d/")
            .replace("\\", "/")
        )
        if not pool.exists():
        ...
        for record in _safe_source_rows(pool):
            if "_audit_source_error" in record:
                source_reader_errors.append(
                    f"{component}: {record['_audit_source_error']}"
                )
                continue
    ...
    for index, item in enumerate(selected):
        ...
        identity_ok = (
            ...
            and selected_identity == derived
            and item.get("complete_identity_sha256") == derived_sha
            and selected_identity.get("complete_identity_sha256")
            == item.get("complete_identity_sha256")
        )
        source_meta = (
            source.get("structural_metadata", {}) if isinstance(source, Mapping) else {}
        )
        selected_source_path = item.get("source_path")
        expected_path = str(
            manifest["input_artifacts"].get(component, {}).get("path", "")
        )
        path_ok = (
        ...
        if teacher_source is not None and not isinstance(teacher_source, Mapping):
            row_problems.append(
                f"row {index}: teacher source_metadata is not an object"
            )
        if (
            isinstance(teacher_source, Mapping)
            and isinstance(teacher_metadata, Mapping)
            and any(
                teacher_source.get(key) != teacher_metadata.get(key)
                for key in ("act", "room_type", "encounter_id", "assistance_level")
                if key in teacher_source
            )
        ):
            row_problems.append(
                f"row {index}: teacher nested provenance conflicts with structural metadata"
            )
        if not isinstance(teacher_metadata, Mapping):
            row_problems.append(
                f"row {index}: teacher structural_metadata is not an object"
            )
        linkage_valid, linkage_problems = _linkage_ok(
            item, teachers[index], trainers[index], index, record_index
        )
        trainer_source_metadata = trainers[index].get("source_metadata")
        if not isinstance(trainer_source_metadata, Mapping):
            row_problems.append(
                f"row {index}: trainer source_metadata is not an object"
            )
        trainer_action = _identity(trainers[index].get("policy_target_action_identity"))
        comparison = (
            "unavailable"
            if behavior.get("status") != "available" or teacher_action is None
            else "same"
            if behavior["identity"] == teacher_action
            else "different"
        )
        ...
        rows.append(
            {
                "index": index,
                "selected_identity": item["complete_identity"],
                "selected_identity_sha256": item.get("complete_identity_sha256"),
                "derived_identity_sha256": derived_sha,
                "identity_valid": identity_ok,
                "identity_error": source_error,
                "component": component,
                "source_record_index": record_index,
                "source_checkpoint_id": source.get("source_checkpoint_id"),
                "source_run_id": source.get("source_run_id"),
                "source_seed": source.get("source_seed"),
                "source_battle_index": source.get("source_battle_index"),
                "act": item.get("act"),
                "room_type": item.get("room_type"),
                "trace_length": len(source.get("action_trace", ()))
                if isinstance(source.get("action_trace", ()), (list, tuple))
                else 0,
                "successor": {
                    "record_index": successor.get("record_index"),
                    "trace_length": len(successor.get("action_trace", ())),
                    "battle_index": successor.get("source_battle_index"),
                }
                if successor
                else None,
                "behavior": behavior,
                "teacher_action": teacher_action,
                "trainer_policy_action": trainer_action,
                "comparison": comparison,
                "outcome": outcome_status,
                "source_battle_outcome": outcome if isinstance(outcome, str) else None,
                "source_controller": source.get("source_battle_controller_provenance"),
                "teacher_controller": teachers[index].get("controller_provenance"),
                "policy_target_source": trainers[index].get("policy_target_source"),
        ...
        trainer_outcome = (
            structured_outcome.get("battle_survived", {})
            if isinstance(structured_outcome, Mapping)
            else {}
        )
        try:
            successor_identity = (
                _record_identity(successor, component)[0] if successor else None
            )
        except (ValueError, KeyError, TypeError, AttributeError) as exc:
        ...
        rows[-1].update(
            {
                ...
                "strict_prefix_validation": {
                    "status": behavior.get("status"),
                    "reason": behavior.get("reason"),
                    "successor_exists": behavior.get(
                ...
                "source_pool": {
                    "component": component,
                    "path": str(manifest["input_artifacts"][component]["path"]),
                    "sha256": manifest["input_artifacts"][component].get("sha256"),
                },
                "current_complete_identity": derived,
                "successor_complete_identity": successor_identity,
                "successor_exists": successor is not None,
                "successor_reason": behavior.get("reason"),
                "source_act": source_meta.get("act")
                if isinstance(source_meta, Mapping)
                else None,
                "source_encounter": source_meta.get("encounter_id")
                if isinstance(source_meta, Mapping)
                else None,
                "source_controller_provenance": source.get(
                    "source_controller_provenance"
                ),
                "source_battle_controller_provenance": source.get(
                    "source_battle_controller_provenance"
                ),
                "source_battle_outcome": outcome
                if isinstance(outcome, str)
                else "unavailable",
                "trainer_battle_survived": {
                    "status": "available"
                    if isinstance(trainer_outcome, Mapping)
                    and "value" in trainer_outcome
                    else "unavailable",
                    "value": trainer_outcome.get("value")
                    if isinstance(trainer_outcome, Mapping)
                    else None,
                },
                ...
                "value_target_source": "trainer_input_record.structured_battle_outcome.battle_survived",
                "raw_reward_components_battle_outcome": raw_rewards.get(
                    "battle_outcome"
                )
        ...
        if not path_ok:
            row_problems.append(
                f"row {index}: selected source_path does not match pool path"
            )
        if not shape_ok:
            row_problems.append(
                f"row {index}: source structural metadata does not match selected metadata"
            )
        if teacher_action is None:
            row_problems.append(
                f"row {index}: teacher action identity missing or malformed"
            )
        if not (
            isinstance(trainer_outcome, Mapping)
            and trainer_outcome.get("status") == "available"
            and isinstance(trainer_outcome.get("value"), bool)
        ):
            row_problems.append(
                f"row {index}: structured battle-survived target unavailable or malformed"
            )
    all_integrity = (
        all(row["identity_valid"] and row["linkage_valid"] for row in rows)
        and all(item["valid"] for item in input_checks)
        and all(item["valid"] for item in pool_checks)
        and terminal_valid
    )
    ...
    coverage_valid = (expected_rows != 460) or (
        observed_acts == Counter({1: 256, 2: 204})
    ...
    row_problems.extend(
        ...
        for row in rows
        if row.get("source_battle_outcome") not in ("unavailable", None)
        and row.get("trainer_battle_survived", {}).get("status") == "available"
        and (
            (row["source_battle_outcome"] == "PLAYER_VICTORY")
            != bool(row["trainer_battle_survived"].get("value"))
        )
    )
    ...
    strata = {
        name: {
            ...
            for key, value in Counter(
                (
                    (
                        _controller_key(row[field])
                        if field == "source_battle_controller_provenance"
                        else row[field]
                    ),
                    row["comparison"],
                )
    ...
    report = {
        ...
        "regeneration": {
            "command": f"python -m sts_combat_rl.t082_value_target_semantic_closure --manifest {manifest_path} --output {output}"
        },
        "inputs": {
            ...
            "terminal_case_valid": terminal_valid,
            "teacher": {"path": str(teacher_path), "sha256": sha256(teacher_path)},
            "trainer": {"path": str(trainer_path), "sha256": sha256(trainer_path)},
            "source_components": {
                component: {
                    "path": str(manifest["input_artifacts"][component]["path"]),
                    "sha256": manifest["input_artifacts"][component].get("sha256"),
        ...
        "coverage": {
            "observed": {str(key): value for key, value in sorted(coverage.items())},
            "expected_acts": {"1": 256, "2": 204},
            "expected_components": {
                "assist_0": 256,
                "assist_hp50": 12,
                "assist_hp50_potion_elite_boss": 32,
        ...
        "integrity": {
            "valid": all_integrity and not row_problems,
            "selected_teacher_trainer_counts": len(rows) == 460,
            "source_identity_valid": all(row["identity_valid"] for row in rows),
            "problems": row_problems
            + (
                []
                if all_integrity
                else [
                    "one or more identity, coverage, control-artifact, or terminal predicates failed"
                ]
            ),
        },
        "counts": {
            "total_rows": len(rows),
            "behavior_recoverable": sum(
                row["behavior"]["status"] == "available" for row in rows
            ),
            "behavior_unavailable": sum(
                row["behavior"]["status"] != "available" for row in rows
            ),
            "comparison_denominator": sum(
                row["comparison"] != "unavailable" for row in rows
            ),
            "comparisons": dict(Counter(row["comparison"] for row in rows)),
            "divergence_rate": len(divergent)
            / max(1, sum(row["comparison"] != "unavailable" for row in rows)),
        ...
        "classification": classify(
            integrity_valid=all_integrity and not row_problems, rows=rows, proof=proof
        ),
    ...
    manifest_decision = next(
        (
            decision
            for decision in input_checks
            if decision.get("name") == "t064-curriculum-manifest.json"
        ),
        None,
    )
    report["inputs"]["manifest"] = {
        ...
        "valid": manifest_decision.get("valid")
        if manifest_decision
        else expected_rows != 460,
    }
    report["inputs"]["manifest"].update(
        {
            "expected_schema": "t064-curriculum-manifest-v1",
            "observed_schema": manifest.get("schema_id"),
            "expected_sha256": EXPECTED_INPUTS.get(
                "t064-curriculum-manifest.json", (None, None)
            )[0],
        }
    )
    report["inputs"]["terminal"] = {
    ...
    report["inputs"]["teacher"]["metadata"] = teacher_envelope_meta
    report["inputs"]["trainer"]["metadata"] = trainer_envelope_meta
    report["inputs"]["teacher"].update(
        {
            "expected_sha256": EXPECTED_TEACHER_SHA,
            "actual_sha256": sha256(teacher_path),
            "valid": input_checks[-2]["valid"],
        }
    )
    report["inputs"]["trainer"].update(
        {
            "expected_sha256": EXPECTED_TRAINER_SHA,
            "actual_sha256": sha256(trainer_path),
            "valid": input_checks[-1]["valid"],
        }
    )
    report["counts"]["trainer_value_labels"] = dict(trainer_outcomes)
    report["counts"]["divergent_trainer_value_labels"] = dict(
        divergent_trainer_outcomes
    )
    report["regeneration"].update(
        {
            "hash_convention": "hash the committed output bytes externally; no self-referential hash is embedded"
        }
    )
    output.write_text(
        json.dumps(report, indent=2, sort_keys=True) + "\n", encoding="utf-8"
...
def classify(
    *, integrity_valid: bool, rows: list[dict[str, Any]], proof: Mapping[str, Any]
) -> str:
    ...
    if (
        ...
        and not aligned
        and divergent
        and any(row.get("outcome") != "unavailable" for row in divergent)
    ):
