...
def _fake_audit(tmp_path: Path, monkeypatch, *, outcomes=None, behavior=False, recorded_path=None):
    ...
    provenance = {"trainer_input_path": recorded_path or "artifacts/trainer.jsonl", "trainer_input_sha256": trainer_sha, "trainer_input_byte_count": trainer.stat().st_size, "trainer_input_format_version": 6, "trainer_record_count": 4, "trainer_input_artifact_id": f"trainer-input-sha256:{trainer_sha}"}
    ...
    return t080.audit(checkpoint, trainer, lambda _: (provenance, {})), checkpoint, trainer
...
def test_outcome_counts_preserve_one_survived_and_three_lost(tmp_path, monkeypatch):
    report, _, _ = _fake_audit(tmp_path, monkeypatch, outcomes=[True, False, False, False])
    assert report["target_lineage"]["outcome_status_counts"] == {"lost": 3, "survived": 1}
