...
| T093 | READY | [Internal-State Partial-Ranking Public Battle-Student Distillation Gate](T093-internal-state-partial-ranking-battle-student.md) | T011, T016, T025, T062, T078, T081, T087, T088, T090, T091, T092 | Candidate public-only distillation gate on the exact T092 depth>=1 `n_min=4` partial-ranking surface with label-destruction and state-ablated controls, inherited leakage-safe splits, and Battle-start-clustered held-out inference; no Search integration, complete-run promotion, Non-Combat update, or T066; implementation requires Maintainer exact-head `SPEC APPROVED`. |
