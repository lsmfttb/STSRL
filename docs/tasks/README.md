...
| T095 | READY | [Repeated-Public-State Oracle Aggregation Feasibility Audit](T095-repeated-public-state-oracle-aggregation-feasibility.md) | T033, T092, T094 | Offline exact-T092 audit treating repeated public states as hidden-future-conditioned observations rather than invalid conflicts; source-start-macro aggregates and deterministic split-half stability test whether a reproducible empirical public action-preference signal exists strongly enough to motivate a separate T034 sampler investment. No Search/simulator run, student training, T034 closure, continuation-value claim, Non-Combat update, or T066. |
...
T095 is the candidate next READY task on this publication branch; until merged,
its open task PR plus exact-head Maintainer approval is the temporary execution
authority. T063 and T066 remain `DRAFT`; T034 remains blocked on native
