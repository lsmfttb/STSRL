...
SHA-256 identities. GB-scale source pools, cohorts, trainer inputs, or
checkpoints may be consumed only through an explicit ignored/local retention
manifest from T042/T043 or regenerated by documented current-`main` commands.
...
hashes, and any lower-worker or single-worker reason per stage. For scale
stages, choose the worker target from the host logical CPU count, capped by
shard count and documented memory/simulator limits; on the current
16-logical-core maintainer machine, use 16 workers by default unless the PR
reports why fewer workers were required.
