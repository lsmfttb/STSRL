...
This document is the Phase-A specification-publication contract. The accompanying Task Index change proposes `READY`, but the task remains non-executable while the publication PR is unmerged.

Publication requires Maintainer exact-head `SPEC APPROVED` with `publication_authorized=true`. After that exact approved publication head is merged, T085 becomes executable and implementation must begin on a fresh implementation branch/PR from the resulting merged `main`. No training, simulator execution, or T085 outcome evaluation is authorized before that merge. A material change to the value-only freeze, training budget, evaluation cohorts, Search arms, or terminal classification requires renewed Planner publication/review.